...
static class G
{
	public static Device d = new(DriverType.Hardware, DeviceCreationFlags.BgraSupport);
	...
	public static SRV CreateSRV(string path) => Textures.LoadTexture(path);
	...
	public static Mesh nearestMesh => Meshes.ElementAtOrDefault(Utilities.Read<int>(Outline.idData.DataPointer) - 1);
	...
	public struct vec1(double x) 
{
	...
	public static vec1 operator +(vec1 a, vec1 b) => new(a.x + b.x);
	public static vec1 operator -(vec1 a, vec1 b) => new(a.x - b.x);
	public static vec1 operator *(vec1 v, int f) => new(v.x * f);
	public static vec1 operator /(vec1 v, int f) => new(v.x / f);
}

...
{	
	...
	//float3 NormalVS = GetViewSpaceNormal(NormalTangentSpace, i.posVS.xyz, normalize(i.normal), i.uv);
	float3 NormalVS = i.normal;
	
	// Diffuse
	float NdotL = clamp(dot(LightDir, NormalVS), 0.06, 1);
	float3 diffuse = NdotL * diffuseColor;
	
	// Specular
	float glossiness = NormalMap.Sample(Sampler, i.uv).z;
	float3 EyeDir = normalize(-i.posVS.xyz);
	float3 R = reflect(-LightDir, NormalVS);

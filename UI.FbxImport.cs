...
public partial class FbxImport : Form
{
	...
	{
		private static string SettingsFilePath = AppContext.BaseDirectory + "\\fbx_import_settings.json";
		...
		public bool MirrorTertiaryAxis = true;
		public bool BlenderTan = false;
		public bool InverseTangentW = false;
	...
	public static bool importFBX(MainForm mainForm)
	{
		...
		Flver.flver.BufferLayouts.Add(newBL);

		int materialCount = Flver.flver.Materials.Count;
		...
		{
			FLVER2.Material matnew = new FLVER2.Material() { Index = Flver.flver.Materials.Count };
			...
			Flver.flver.Materials.Add(matnew);
		}
		...
		foreach (var m in md.Meshes)
		{
			...
			if (m.HasBones)
			{
				...
				for (int i2 = 0; i2 < m.BoneCount; i2++)
				{
					...
					boneIndex = findFLVER_Bone(Flver.flver, boneName);

...
public partial class FbxImport : Form
{
	...
	public FbxImport(FbxImportSettings initialSettings)
	{
	...
	public static bool ImportFBX()
	{
		...
		using (var form = new FbxImport(settings))
		{
	...
	//	Directory.SetCurrentDirectory(Path.GetDirectoryName(settings.ImportFilePath));
		...
		Scene scene = importer.ImportFile(settings.ImportFilePath, PostProcessSteps.CalculateTangentSpace | PostProcessSteps.Triangulate); // PostProcessPreset.TargetRealTimeMaximumQuality
		...
		{
			new SoulsFormats.FLVER.LayoutMember(SoulsFormats.FLVER.LayoutType.Float3, SoulsFormats.FLVER.LayoutSemantic.Position, 0), // 2
			new SoulsFormats.FLVER.LayoutMember(SoulsFormats.FLVER.LayoutType.UByte4, SoulsFormats.FLVER.LayoutSemantic.Normal, 0), // 
			new SoulsFormats.FLVER.LayoutMember(SoulsFormats.FLVER.LayoutType.UByte4, SoulsFormats.FLVER.LayoutSemantic.Tangent, 0),
			new SoulsFormats.FLVER.LayoutMember(SoulsFormats.FLVER.LayoutType.UByte4, SoulsFormats.FLVER.LayoutSemantic.Tangent, 1),
			new SoulsFormats.FLVER.LayoutMember(SoulsFormats.FLVER.LayoutType.UByte4, SoulsFormats.FLVER.LayoutSemantic.BoneIndices, 0),
			new SoulsFormats.FLVER.LayoutMember(SoulsFormats.FLVER.LayoutType.UByte4Norm, SoulsFormats.FLVER.LayoutSemantic.BoneWeights, 0),
			new SoulsFormats.FLVER.LayoutMember(SoulsFormats.FLVER.LayoutType.UByte4Norm, SoulsFormats.FLVER.LayoutSemantic.VertexColor, 1),
			new SoulsFormats.FLVER.LayoutMember(SoulsFormats.FLVER.LayoutType.Short4, SoulsFormats.FLVER.LayoutSemantic.UV, 0)
		};
		...
		{
			string albedo = Path.GetFileNameWithoutExtension(fbxMaterial.TextureDiffuse.FilePath).ToLower();
			string baseName;

			if (albedo.Contains("_d"))
				baseName = albedo.Replace("_d", "");
			else if (albedo.Contains("_a"))
				baseName = albedo.Replace("_a", "");
			else
				baseName = albedo;

			FLVER2.Material material = new FLVER2.Material()
			{
				Name = fbxMaterial.Name,
				MTD = "C[AMSN]_e.matxml",
				Index = flver.Materials.Count
			};

			material.Textures.Add(new() { Path = albedo, Type = "C_AMSN__snp_Texture2D_2_AlbedoMap_0" });
			material.Textures.Add(new() { Path = baseName + "_n", Type = "C_AMSN__snp_Texture2D_7_NormalMap_4" });
		...
		foreach (var m in scene.Meshes)
		{
			...
			mn.Vertices = new List<SoulsFormats.FLVER.Vertex>();
			...
			if (m.HasBones)
			{
				...
				for (int i2 = 0; i2 < m.BoneCount; i2++)
				{
					...
					boneIndex = findFLVER_Bone(flver, boneName);
			...
			for (int i = 0; i < m.Vertices.Count; i++)
			{
				...
				if (m.HasTangentBasis && m.Tangents.Count > i)
				{
					...
					if (swapTanBitan) { tangent = m.BiTangents[i]; }
				...
				else if (m.HasNormals && m.Normals.Count > i)
				{
					...
					normal.Normalize();
					tangent = Vector3D.Cross(m.Normals[i], normal);
				}
				...
				if (m.HasTangentBasis && m.BiTangents.Count > i)
				{
					...
					m.BiTangents[i].Normalize();
					hasBitangent = true;
					bitangent = m.BiTangents[i];
					if (swapTanBitan) { bitangent = m.Tangents[i]; }
				}
				else
				{
					bitangent = tangent;
				...
				Vector3 remappedPosition = RemapVector(vit, settings.PrimaryAxis, settings.SecondaryAxis, settings.MirrorTertiaryAxis);
				...
				if (hasBitangent)
				{
					Vector3D n = normal;
					Vector3D t = tangent;
					Vector3D positiveBitangent = Vector3D.Cross(n, t);
					if (Vector3D.Dot(positiveBitangent, bitangent) < 0)
					{
						tangentW *= -1;
					}
				}

				SoulsFormats.FLVER.Vertex v = generateVertex(remappedPosition, uv1, uv2, remappedNormal, remappedTangent, tangentW);
...
/*
	...
	static SoulsFormats.FLVER.Vertex generateVertex(Vector3 pos, Vector3D uv1, Vector3D uv2, Vector3 normal, Vector3 tangets, float tangentW = -1)
	{
		SoulsFormats.FLVER.Vertex ans = new SoulsFormats.FLVER.Vertex
		{
			Position = pos
		};
		...
		{
			new(uv1.X, uv1.Y, uv1.Z),
			new(uv2.X, uv2.Y, uv2.Z)
		};
		ans.Normal = normal;
		ans.NormalW = -1;
		...
		ans.Colors = new List<SoulsFormats.FLVER.VertexColor>
		{
			new SoulsFormats.FLVER.VertexColor(255, 255, 255, 255)
		};
		return ans;
	}

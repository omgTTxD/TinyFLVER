...
{
	public FbxImportSettings Settings;
	...
	{
		var ofd = new OpenFileDialog { Filter = "FBX Files|*.fbx|All Files|*.*" };
	...
	public static bool importFBX(MainForm mainForm)
	{
		...
		flver.BufferLayouts.Add(new FLVER2.BufferLayout {
			new (FLVER.LayoutType.Float3, FLVER.LayoutSemantic.Position),
			new (FLVER.LayoutType.UByte4, FLVER.LayoutSemantic.Normal),
			new (FLVER.LayoutType.UByte4, FLVER.LayoutSemantic.Tangent),
			new (FLVER.LayoutType.UByte4, FLVER.LayoutSemantic.BoneIndices),
			new (FLVER.LayoutType.UByte4Norm, FLVER.LayoutSemantic.BoneWeights),
			new (FLVER.LayoutType.Short4, FLVER.LayoutSemantic.UV)
		});
		...
		foreach (var mat in md.Materials)
		{
			...
			if (settings.SetTexture)
			{
				if (mat.HasTextureDiffuse) 
					SetFlverMatPath(matnew, "g_DiffuseTexture", mat.TextureDiffuse.FilePath);
				if (mat.HasTextureNormal)
					SetFlverMatPath(matnew, "g_BumpmapTexture", mat.TextureNormal.FilePath);
				if (mat.HasTextureSpecular)
					SetFlverMatPath(matnew, "g_SpecularTexture", mat.TextureSpecular.FilePath);
			}
		...
		foreach (var m in md.Meshes)
		{
			...
			mn.VertexBuffers.Add(new FLVER2.VertexBuffer(layoutIndex: 0));
			...
			if (m.HasBones)
			{
				...
				for (int i2 = 0; i2 < m.BoneCount; i2++)
				{
					...
					{
						if (boneIndex == -1 && boneParentList.ContainsValue(boneName) && boneParentList[boneName] != null)
						{
			...
			for (int i = 0; i < m.Vertices.Count; i++)
			{
				...
				if (channels != null && m.TextureCoordinateChannelCount > 0)
				{
					...
					while (tempUvs.Count < 2) { 
						tempUvs.Add(new Vector3(uv1.X, uv1.Y, uv1.Z)); 
					}
				}
				...
				if (inverseTan) { 
					remappedTangent = new Vector3(-remappedTangent.X, -remappedTangent.Y, -remappedTangent.Z); 
				}
				...
				if (tempUvs.Count > 1) { v.UVs = tempUvs; }
				if (m.HasBones)
				{
	...
	{
		SoulsFormats.FLVER.Vertex ans = new SoulsFormats.FLVER.Vertex { Position = pos };
...
public class FbxImportSettings
{
	...
	public Axis primaryAxis = Axis.X;
	public Axis secondaryAxis = Axis.Y;
	public bool mirrorTertiaryAxis;
	public bool blenderTan;
	public bool inverseTangentW;
	public bool SetTexture;

	public void Save()
	{
		string json = JsonConvert.SerializeObject(this);
		File.WriteAllText(AppContext.BaseDirectory + "\\ImportSettings.json", json);
	}
	...
	public static FbxImportSettings Load()
	{
		try
		{
			string json = File.ReadAllText(AppContext.BaseDirectory + "\\ImportSettings.json");
			return JsonConvert.DeserializeObject<FbxImportSettings>(json);
		}
		catch { return new FbxImportSettings(); }
	}

...
public class Yabber
{
	...
	static void Repack(string path)
	{
		...
		string outPath = $"{Directory.GetParent(Path.GetDirectoryName(path))}\\{node.Attributes["filename"].InnerText}";
		string backupPath = outPath.EndsWith(".dcx") ? outPath.Replace(".dcx", "_.dcx") : outPath.Replace(".tpf", "_.tpf");
	...
	static TPF RepackTPF(XmlNode node)
	{
		...
		foreach (string path in Directory.GetFiles(".", "*.dds"))
		{
			...
			//	tpf.Textures.Add(new TPF.Texture(fileName, 0, 0, File.ReadAllBytes(fileName + ".dds")));

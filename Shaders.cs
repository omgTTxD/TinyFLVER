...
class Shaders
{
	...
	public static void Initialize()
	{
		...
		form.RecalculateTangents.CheckedChanged += (s, e) => { shaderData.RecalcTan = form.RecalculateTangents.Checked ? true : false; form.comboBoxShaders.Focus(); };
		...
		c.PixelShader.SetSampler(1, new(d, SamplerStateDescription.Default() with
		{
	...
	static void InitializeShadersListboxes()
	{
		...
		Where(x => x.StartsWith("Debug") || x.StartsWith("PS")).ToArray();
		...
		{
			if (!PSs.ContainsKey(form.comboBoxShaders.Text))
				PSs[form.comboBoxShaders.Text] = new(d, Compile(form.comboBoxShaders.Text, "PS"));
			
			PS = PSs[form.comboBoxShaders.Text];
			BindDebugList();
		};
	...
	static void BindDebugList()
	{
		...
		if (shaderName.StartsWith("PS.PBR"))
			form.comboBoxDebugID.DataSource = Desc.debugPBR;

		if (shaderName.StartsWith("Debug.Normals"))
			form.comboBoxDebugID.DataSource = Desc.debugNormals;

		if (index < form.comboBoxDebugID.Items.Count)

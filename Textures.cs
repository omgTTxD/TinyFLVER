using Pfim;
using SharpDX;
using SharpDX.Direct3D11;
using SharpDX.DXGI;
using SoulsFormats;
using System.Collections.Concurrent;

namespace TinyFLVER;

public abstract class Textures
{
	public static ShaderResourceView defaultDiffuseSRV = new (dev, CreateDefaultTextures([200, 200, 200, 255]));
	public static ShaderResourceView defaultNormalSRV = new(dev, CreateDefaultTextures([127, 127, 127, 255]));

	static ConcurrentDictionary<string, ShaderResourceView> Cache = [];

	public static Texture2D CreateDefaultTextures(byte[] data) {
		var desc = new Texture2DDescription {
			Width = 1,
			Height = 1,
			ArraySize = 1,
			Format = Format.R8G8B8A8_UNorm,
			BindFlags = BindFlags.ShaderResource,
			SampleDescription = new SampleDescription(1, 0)
		};
		
		var dataStream = DataStream.Create(data, true, false);
		var dataRect = new DataRectangle(dataStream.DataPointer, 4);
		return new Texture2D(dev, desc, dataRect);
	}

	public static ShaderResourceView LoadDDS(FLVER2.Texture texture, Format format = Format.B8G8R8A8_UNorm) {
		string textureName = Path.GetFileNameWithoutExtension(texture?.Path);
		string fullPath = Path.GetDirectoryName(path) + $"\\{textureName}.dds";

		if (!File.Exists(fullPath)) 
			return null;

		// If checkBox on form is disabled, we use non-srgb format for diffuse textures
		if (form.checkBoxDiffuseSRgb.Checked == false)
			format = Format.B8G8R8A8_UNorm;

		string key = textureName + "|" + format;

		if (Cache.ContainsKey(key))
			return Cache[key];

		var fileStream = new FileStream(fullPath, FileMode.Open, FileAccess.Read);

		var image = Dds.Create(File.ReadAllBytes(fullPath), new ());

		var desc = new Texture2DDescription
		{
			Width = image.Width, 
			Height = image.Height,
			Format = format,
			MipLevels = 1,
			ArraySize = 1,
			SampleDescription = new(1, 0),
			BindFlags = BindFlags.ShaderResource,
		};

		// For normal maps we should create mips. Otherwise skin will have sharp pixelated look. 
		// Also for this reason we don't create SamplerState, because default filtering is good (unlike Anisotropic and others)
		if (textureName.ToLower().Contains("_n"))
		{
			desc.MipLevels = 3;
			desc.OptionFlags = ResourceOptionFlags.GenerateMipMaps;
			desc.BindFlags |= BindFlags.RenderTarget;
		}

		var tex = new Texture2D(dev, desc);
		c.UpdateSubresource(image.Data, tex, 0, image.Width * 4);

		var SRV = new ShaderResourceView(dev, tex);

		// Generate mip for normal maps
		if (textureName.Contains("_n")) 
			c.GenerateMips(SRV);

		Cache[key] = SRV;
		return SRV;
	}

	public static void Load(Mesh mesh )
	{
		Task.Run(() =>
		{
			var textures = mesh.material.Textures;
			textures.ForEach(x => x.Path = x.Path.ToLower());

			var d = textures.Find(x => x.Path.Contains("_a") || x.Path.Contains("_d") || x.Type.Contains("diffuse") || x.Type.Contains("albedo"));
			if (d is not null && (d.Path.Contains("hair") || d.Path.Contains("brow") || d.Path.Contains("eyelashes"))) mesh.Transparent = true;
			mesh.diffuseBinding = LoadDDS(d, Format.B8G8R8A8_UNorm_SRgb);

			mesh.normalBinding = LoadDDS(textures.Find(x => x.Path.Contains("_n")));
			mesh.sssBinding = LoadDDS(textures.Find(x => x.Path.Contains("_m") || x.Type.Contains("sss")));
		});
	}
}